        ...
        for row in cursor.fetchall():
            ...
            cursor_prod.execute("""
                ...
                GROUP BY p.ProductoId, p.CodigoUnico, p.NombreProducto, p.Descripcion,
                         p.CantidadDisponible, p.ValorUnitario, p.CantidadMinima, p.Modelo, p.Serial
            """, (oficina['id'],))
            ...
            for prod_row in cursor_prod.fetchall():
                material = {
                    ...
                    'modelo': prod_row[7] or '',
                    'serial': prod_row[8] or '',
                    'categoria': prod_row[9] or 'Corporativo',
                    'estado': 'Activo',
                    'unidad': 'Unidad',
...
def _puede_ver_reporte_asignaciones_persona_global():
    rol = (session.get('rol') or '').strip().lower()
    for origen, destino in (('á', 'a'), ('é', 'e'), ('í', 'i'), ('ó', 'o'), ('ú', 'u'), ('ü', 'u'), ('ñ', 'n')):
        rol = rol.replace(origen, destino)
    rol = rol.replace(' ', '_')
    roles_globales = {'administrador', 'lider_inventario', 'aprobador', 'admin'}
    return rol in roles_globales


def _consultar_asignaciones_persona(filtros=None, solo_propias=False):
    filtros = filtros or {}
    conn = get_database_connection()
    if conn is None:
        return []
    ...
    if solo_propias:
        ...
        usuario_login = (session.get('usuario') or '').strip()
        usuario_nombre = (session.get('usuario_nombre') or '').strip()

        propias = []
        propios_params = []
        if usuario_id:
            propias.append("a.UsuarioAsignadoId = ?")
            propios_params.append(usuario_id)
        if usuario_login:
            propias.append("LOWER(LTRIM(RTRIM(COALESCE(a.UsuarioADEmail, '')))) = LOWER(LTRIM(RTRIM(?)))")
            propios_params.append(usuario_login)
        if usuario_nombre:
            propias.append("LOWER(LTRIM(RTRIM(COALESCE(a.UsuarioADNombre, '')))) = LOWER(LTRIM(RTRIM(?)))")
            propios_params.append(usuario_nombre)

        if propias:
            condiciones.append('(' + ' OR '.join(propias) + ')')
            params.extend(propios_params)
        else:
            condiciones.append('1 = 0')

    nombre = (filtros.get('nombre') or '').strip()
    correo = (filtros.get('correo') or '').strip()
    oficina_id = (filtros.get('oficina_id') or '').strip()
    estado = (filtros.get('estado') or '').strip()

    if nombre:
        condiciones.append("a.UsuarioADNombre LIKE ?")
        params.append(f"%{nombre}%")
    if correo:
        condiciones.append("a.UsuarioADEmail LIKE ?")
        params.append(f"%{correo}%")
    if oficina_id:
        try:
            condiciones.append("a.OficinaId = ?")
            params.append(int(oficina_id))
        except Exception:
            pass
    if estado:
        condiciones.append("UPPER(LTRIM(RTRIM(a.Estado))) = ?")
    ...
    query = f"""
        SELECT
            ...
            p.CodigoUnico,
            p.NombreProducto,
            CAST(COALESCE(p.ValorUnitario, 0) AS DECIMAL(18,2)) AS ValorUnitario,
            CAST(1 AS INT) AS Cantidad,
            CAST(COALESCE(p.ValorUnitario, 0) AS DECIMAL(18,2)) AS ValorTotal,
            p.Serial,
            p.Modelo,
        ...
        LEFT JOIN Oficinas o ON a.OficinaId = o.OficinaId
        WHERE {where_clause}
        ORDER BY a.UsuarioADNombre ASC, a.FechaAsignacion DESC, a.AsignacionId DESC
    """
...
@reportes_bp.route('/asignaciones-persona')
def reporte_asignaciones_persona():
    if not _require_login():
        return redirect(url_for('auth.login'))

    if not _can_view_reportes():
        flash('No tiene permisos para ver este reporte', 'warning')
        return redirect('/reportes')

    puede_ver_todo = _puede_ver_reporte_asignaciones_persona_global()
    filtros = {
    ...
    asignaciones = _consultar_asignaciones_persona(filtros, solo_propias=not puede_ver_todo)
    oficinas = OficinaModel.obtener_todas() or []

    total_asignaciones = len(asignaciones)
    total_confirmadas = len([a for a in asignaciones if (a.get('Estado') or '').strip().upper() == 'CONFIRMADO'])
    total_pendientes = len([a for a in asignaciones if (a.get('Estado') or '').strip().upper() == 'ASIGNADO'])
    total_devueltas = len([a for a in asignaciones if (a.get('Estado') or '').strip().upper() == 'DEVUELTO'])
    total_cantidad = sum(int(a.get('Cantidad') or 0) for a in asignaciones)
    valor_total_asignado = sum(float(a.get('ValorTotal') or 0) for a in asignaciones)

    return render_template(
        'reportes/asignaciones_persona.html',
        ...
        valor_total_asignado=valor_total_asignado,
    )


@reportes_bp.route('/asignaciones-persona/exportar/excel')
def exportar_asignaciones_persona_excel():
    if not _require_login():
        return redirect(url_for('auth.login'))
    if not _can_view_reportes():
        flash('No tiene permisos para exportar este reporte', 'warning')
        return redirect('/reportes')
    ...
    for a in asignaciones:
        data.append({
            'Asignacion ID': a.get('AsignacionId'),
            'Nombre Persona': a.get('UsuarioADNombre', ''),
            'Correo': a.get('UsuarioADEmail', ''),
            'Oficina': a.get('NombreOficina', ''),
            'Producto': a.get('NombreProducto', ''),
            'Codigo': a.get('CodigoUnico', ''),
            'Categoria': a.get('Categoria', ''),
            'Modelo': a.get('Modelo', ''),
            'Serial': a.get('Serial', ''),
            'Cantidad': int(a.get('Cantidad') or 0),
            'Valor Unitario': float(a.get('ValorUnitario') or 0),
            'Valor Total': float(a.get('ValorTotal') or 0),
            'Estado': a.get('Estado', ''),
            'Fecha Asignacion': a.get('FechaAsignacion').strftime('%Y-%m-%d %H:%M') if a.get('FechaAsignacion') else '',
            'Fecha Confirmacion': a.get('FechaConfirmacion').strftime('%Y-%m-%d %H:%M') if a.get('FechaConfirmacion') else '',
            'Fecha Devolucion': a.get('FechaDevolucion').strftime('%Y-%m-%d %H:%M') if a.get('FechaDevolucion') else '',
            'Usuario Asignador': a.get('UsuarioAsignador', ''),
            'Usuario Confirmacion': a.get('UsuarioConfirmacion', ''),
    ...
    return send_file(
        ...
        mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
        as_attachment=True,
        download_name=f"reporte_asignaciones_persona_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
    )


@reportes_bp.route('/asignaciones-persona/exportar/pdf')
def exportar_asignaciones_persona_pdf():
    if not _require_login():
        return redirect(url_for('auth.login'))
    if not _can_view_reportes():
        flash('No tiene permisos para exportar este reporte', 'warning')
    ...
    output = BytesIO()
    doc = SimpleDocTemplate(output, pagesize=landscape(letter), leftMargin=18, rightMargin=18, topMargin=24, bottomMargin=24)
    styles = getSampleStyleSheet()
    titulo_style = ParagraphStyle(
        'AsignacionesPersonaTitulo',
        parent=styles['Title'],
        textColor=colors.HexColor('#A73493'),
    ...
    elements = [Paragraph('Reporte de Asignaciones por Persona', titulo_style)]
    subtitulo = 'Vista global de asignaciones' if puede_ver_todo else f"Asignaciones del usuario: {session.get('usuario_nombre', '')}"
    elements.append(Paragraph(subtitulo, subtitulo_style))
    elements.append(Paragraph(
        f"Cantidad total de elementos asignados: <b>{total_cantidad}</b> &nbsp;&nbsp;&nbsp; Valor total asignado: <b>${valor_total_asignado:,.2f} COP</b>",
        subtitulo_style,
    ))
    elements.append(Spacer(1, 8))

    encabezados = [
        'Persona', 'Correo', 'Oficina', 'Producto', 'Codigo', 'Categoria', 'Modelo', 'Serial', 'Cant.', 'Vlr Unit.', 'Vlr Total', 'Estado'
    ]

    cell_style = ParagraphStyle(
        'AsignacionesPersonaCell',
    ...
    cell_style_center = ParagraphStyle(
        'AsignacionesPersonaCellCenter',
        parent=cell_style,
        alignment=1,
    )
    cell_style_right = ParagraphStyle(
        'AsignacionesPersonaCellRight',
        parent=cell_style,
        alignment=2,
    )
    header_cell_style = ParagraphStyle(
        'AsignacionesPersonaHeaderCell',
        parent=styles['BodyText'],
        fontName='Helvetica-Bold',
        fontSize=7,
        leading=8,
    ...
    table_data = [[Paragraph(escape(h), header_cell_style) for h in encabezados]]
    for a in asignaciones:
        table_data.append([
            Paragraph(escape(a.get('UsuarioADNombre', '') or ''), cell_style),
            Paragraph(escape(a.get('UsuarioADEmail', '') or ''), cell_style),
            Paragraph(escape(a.get('NombreOficina', '') or ''), cell_style),
            Paragraph(escape(a.get('NombreProducto', '') or ''), cell_style),
            Paragraph(escape(a.get('CodigoUnico', '') or ''), cell_style_center),
            Paragraph(escape(a.get('Categoria', '') or ''), cell_style),
            Paragraph(escape(a.get('Modelo', '') or ''), cell_style),
            Paragraph(escape(a.get('Serial', '') or ''), cell_style),
            Paragraph(str(int(a.get('Cantidad') or 0)), cell_style_center),
            Paragraph(escape(f"${float(a.get('ValorUnitario') or 0):,.0f}"), cell_style_right),
            Paragraph(escape(f"${float(a.get('ValorTotal') or 0):,.0f}"), cell_style_right),
            Paragraph(escape(a.get('Estado', '') or ''), cell_style_center),
        ])

    table = Table(
        table_data,
        repeatRows=1,
        colWidths=[1.0*inch, 1.45*inch, 0.9*inch, 1.05*inch, 0.62*inch, 0.72*inch, 0.72*inch, 0.72*inch, 0.45*inch, 0.78*inch, 0.82*inch, 0.67*inch],
    )
    table.setStyle(TableStyle([
        ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#0098B1')),
    ...
    return send_file(
        ...
        mimetype='application/pdf',
        as_attachment=True,
        download_name=f"reporte_asignaciones_persona_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
    )
